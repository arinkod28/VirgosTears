  ...
  return (
    <>
      ...
      <li
        ...
        className={cn(
          "icon group/li flex h-[var(--icon-size)] w-[var(--icon-size)] cursor-pointer items-center justify-center px-[calc(var(--icon-size)*0.075)] hover:-mt-[calc(var(--icon-size)/2)] hover:h-[calc(var(--icon-size)*1.5)] hover:w-[calc(var(--icon-size)*1.5)]",
          className
        )}
      >
        <a
          href={href}
          className={cn(
            "group/a relative aspect-square w-full rounded-[10px] border bg-gradient-to-t p-1.5 shadow-[rgba(0,_0,_0,_0.05)_0px_1px_0px_inset] after:absolute after:inset-0 after:rounded-[inherit] after:shadow-md after:shadow-zinc-800/10",
            active
              ? "border-blue-400 from-blue-600/20 to-blue-500/20"
              : "border-gray-100 from-neutral-100 to-white dark:border-zinc-900 dark:from-zinc-900 dark:to-zinc-800"
          )}
        >
          ...
          <div className={cn(
            "h-full w-full rounded-[inherit] flex items-center justify-center",
            active ? "text-blue-400" : "text-slate-600 group-hover/a:text-slate-900 dark:text-slate-400"
          )}>
            {icon || children}
          </div>
        </a>
...
export function Dock({
  ...
  return (
    <nav ref={dockRef} role="navigation" aria-label="Main Dock" className="flex justify-center items-center py-4">
      <ul
        className={cn(
          "flex items-center gap-2 rounded-xl border border-gray-100 bg-gradient-to-t from-neutral-50 to-white p-2 dark:border-zinc-900 dark:from-zinc-950 dark:to-zinc-900",
          className
        )}
      >
